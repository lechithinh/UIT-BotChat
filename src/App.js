...
function App() {
  ...
  const Actions = {
    setName: (newName) => {
      dataRef.current.name = newName;
    },
    setUid: (newUid) => {
      dataRef.current.uid = newUid;
    },
    setPath: (newPath) => {
      dataRef.current.path = newPath;
    },
    setDaySchedule: (newDaySchedule) => {
      dataRef.current.DaySchedule = [].concat(newDaySchedule);
    },
    setWeekSchedule: (newWeekSchedule) => {
      dataRef.current.WeekSchedule = [].concat(newWeekSchedule);
    },
    setStatus: (newStatus) => {
      dataRef.current.Status = newStatus;
    },
