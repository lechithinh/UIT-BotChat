...
const UserRow = (props, ref) => {
  ...
  const HandleSaveButton = async () => {
    ...
    if (getNameData.code === 0 || getNameData.data.hoten === null) {
      //ID không hợp lệ
      ...
      //Call API Schedule
      const schedule = await GetSchedule(props.user.uid);
  ...
  return (
    <>
      ...
      <ListItem
        sx={{ backgroundColor: props.user.working ? "#DEE9F3" : "white" }}
      >
        {/* AVATATAR */}
        <ListItemAvatar>
          <Avatar src={props.user.path} sx={{ width: 70, height: 70 }} />
        </ListItemAvatar>
        ...
        <ListItemText
          ...
          sx={{
            ...
            "& .MuiTypography-root": { fontSize: "26px", fontWeight: 600 },
          }}
        />
          ...
          >
            {showEditIcon ? (
              <>
                <Fab
                  variant="extended"
                  color={props.user.Status === INVALID_ID ? "error" : "default"}
                ...
                <Fab
                  ...
                  onClick={CheckTKB}
                  disabled={
                    props.user.Status === INVALID_ID ||
                    props.user.name === "Người mới" ||
                    props.user.uid === ""
                      ? true
                      : false
                  }
      ...
      {showAlert && (
        <Alert
          sx={{
            mt: 2,
            mb: 2,
            "& .MuiAlert-message": { fontSize: "1.7rem" },
            "& .MuiAlert-icon": { marginTop: "4px", fontSize: "35px" },
          }}
          variant="standard"
